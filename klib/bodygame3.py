...
class BodyGameRuntime(object):
    ...
    def __init__(self, info, exe_num=3, if_recording=False):
        ...
        if self.kp.kinect:
            ...
            if self._kinect.has_new_color_frame():
                frame = self._kinect.get_last_color_frame().reshape([self.height, self.width, 4])[:, :,
                        :3]  # can be None
    ...
    def init_param(self, clean=False):
        ...
        if self.kp.scene_type == 2:
            self.ori = (int(self._screen.get_width() * self.kp.video_LB / 1920.),
                        int(self._screen.get_height() * self.kp.video2_UB / 1080.))
        else:
            self.ori = (int(self._screen.get_width() * self.kp.video_LB / 1920.),
                        int(self._screen.get_height() * self.kp.video1_UB / 1080.))
        ...
        if press[pygame.K_z]:  # use 'z' to lower the ratio of avatar to color frame
            ...
            if press[pygame.K_LCTRL] or press[pygame.K_RCTRL]:
                if self.kp.ratio <= 0.6:
                    self.kp.ratio += 0.05
                    if self.kp.kinect:
                        self.kp.scale = self.movie.ini_resize(self._screen.get_width(), self._screen.get_height(),
                                                              self.kp.ratio)
            else:
    ...
    def process_analysis(self):
        ...
        if self.ana.ongoing_cycle == False:  # was if self.ana.shld.ongoing_cycle == False or
            if 'well' in (self.ana.evalstr).lower():  # if well exists in immediate feedback
                ...
                self.eval.blit_text(self.bk_frame_surface, self.exeno, self.kp, self.ana.screen_message, 3,
            ...
            if len(self.evalhis) != 0:
                if self.ana.exercise_started:
                    ...
                    #                'Overall evaluation:\nOnly need to do 4 repetitions.', 3, color=self.kp.c_togo, fsize=70) #font size is 80
                        ...
                        self.eval.blit_text(self.bk_frame_surface, self.exeno, self.kp, \
                                            'Make sure you do 4 repetitions.', 3, color=self.kp.c_togo,
                                            fsize=70)  # font size is 80
                else:
                    self.eval.blit_text(self.bk_frame_surface, self.exeno, self.kp, \
                                        'Overall evaluation:\nNo repetition done', 3, color=self.kp.c_togo,
                                        fsize=70)  # font size is 80
        else:
            self.ana.screen_message = ''
            self.eval.blit_text(self.bk_frame_surface, self.exeno, self.kp, \
                                'Overall evaluation:\n- ' + self.errsums, 3, color=self.kp.c_togo,
                                fsize=70)  # font size was 80

        # the main menu after each exercise
        self.eval.blit_text(self.bk_frame_surface, self.exeno, self.kp, \
                            'Next exercise will start in %s seconds.' % str(self.cntdown / 30), 0, (120, 800),
                            color=self.kp.c_guide, fsize=50)  # the color was color=self.kp.c_togo
        self.eval.blit_text(self.bk_frame_surface, self.exeno, self.kp, \
                            'Or press "Space" to start next exercise \nOr press ESC to Quit.', 0, (120, 840),
                            color=self.kp.c_guide, fsize=50)  # the color was color=self.kp.c_togo
    ...
    def run_body_game(self):
            ...
            if self._bodies is not None:
                ...
                if (closest_ID != -1):
                    ...
                    if self.kp.handmode and frame is not None:  # finger detect and draw
                        ...
                        self.fextr.run(frame, bkimg, body, bddic, jps, pygame.color.THECOLORS["yellow"],
                                       self._frame_surface)
        ...
        if not self.ana._done:
            if self.exercise_initialized:  # exercise alread started, display pause and stop only
                ...
                # button2 = pause
                pygame.draw.rect(self.bk_frame_surface, self.kp.c_button_pause,
                                 pygame.Rect(self.kp.button_left2, self.kp.button_top, self.kp.button_w,
                                             self.kp.button_h))
                # button3 = stop
                pygame.draw.rect(self.bk_frame_surface, self.kp.c_button_stop,
                                 pygame.Rect(self.kp.button_left3, self.kp.button_top, self.kp.button_w,
                                             self.kp.button_h))

                # draw text on buttons
                # self.text_to_button("start", self.kp.button_left1, self.kp.button_top, self.kp.button_w , self.kp.button_h ) #900, 40
                if self.paused:
                    self.text_to_button("continue", self.kp.button_left2, self.kp.button_top, self.kp.button_w,
                                        self.kp.button_h)  # 1110, 40
                else:
                    self.text_to_button("pause", self.kp.button_left2, self.kp.button_top, self.kp.button_w,
                                        self.kp.button_h)  # 1110, 40
                self.text_to_button("stop", self.kp.button_left3, self.kp.button_top, self.kp.button_w,
                                    self.kp.button_h)  # 1320, 40

            else:  # exercise did not start yet, display start only
                # button1 = start
                pygame.draw.rect(self.bk_frame_surface, self.kp.c_button_play,
                                 pygame.Rect(self.kp.button_left1, self.kp.button_top, self.kp.button_w,
                                             self.kp.button_h))
                # button2 = pause
                ...
                self.text_to_button("start", self.kp.button_left1, self.kp.button_top, self.kp.button_w,
                                    self.kp.button_h)  # 900, 40
        ...
        if self.kp.vid_rcd:  # video recoding text
            ...
            if 'frame' in locals():
                ...
                print
                'Color frames unfound... frame number:', self.kp.fno
            # ! change
            # uncomment original ones
            if bodyidx is not None:
                print('+=+' * 50)
                self.bdimgs.create_dataset('bd_' + repr(self.kp.fno).zfill(4),
                                           data=np.dstack((bodyidx, bodyidx, bodyidx)))
            ...
            if 'joints' in locals():
                ...
                print
                'Joints unfound! frame number:', self.kp.fno
            self.kp.fno += 1
        ...
        # draw background
        bksurface_to_draw = pygame.transform.scale(self.bk_frame_surface,
        ...
        # emoji
        emoji_size = min(int(self._screen.get_width() * 120. / self.width),
                         int(self._screen.get_height() * 120. / self.height))
        ...
        surface_to_draw = pygame.transform.scale(self._frame_surface,
                                                 (int(self.kp.vid_w * scale), int(self.kp.vid_h * scale)))
